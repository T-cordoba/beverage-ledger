...
export default function MovementCard({
  ...
  const handlePDFDownload = () => {
    ...
    window.open(
      `/api/movimientos/${movimiento.id}/pdf?date=${encodeURIComponent(formattedDate)}`,
      '_blank',
    );
  };
      ...
      <div className="p-4 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between sm:gap-2">
          <div className="flex-1 min-w-0">
            <h3 className="text-lg sm:text-xl font-medium text-accent mb-1">Movement</h3>
        ...
        <div className="space-y-2 mt-4">
          ...
          >
            {displayLiquors.map((licor, index) => (
              <div
                ...
                className="flex justify-between items-start py-3 border-b border-white/5 last:border-b-0"
              >
          ...
          {hasMoreLiquors && (
            ...
            >
              <span>
                {isExpanded ? `Show less` : `Show ${movimiento.liquors.length - 3} more items`}
              </span>
              <svg
