...
export default function SelectionSection({
  ...
  const uniqueOrigins = Array.from(
    new Set(licores.map((licor) => licor.origin).filter(Boolean)),
  ).sort();
  const uniqueSubcategories = Array.from(
    new Set(licores.map((licor) => licor.subcategory).filter(Boolean)),
  ).sort();
  const uniqueAges = Array.from(new Set(licores.map((licor) => licor.age).filter(Boolean))).sort();
  const uniqueAbvs = Array.from(
  ...
  const hasSelectedItems = Object.values(cantidades).some(
    (cantidad) => cantidad.botellas > 0 || cantidad.cajas > 0,
  );
        ...
        <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 lg:gap-6">
          <div className="relative flex-1 max-w-full sm:max-w-md">
            ...
            <div className="absolute right-3 sm:right-4 top-1/2 transform -translate-y-1/2">
              {searchTerm ? (
                <ClearSearchButton onClick={() => onSearchChange('')} aria-label="Clear search">
                  ×
                </ClearSearchButton>
              ) : (
                <svg
        ...
        <ul className="grid gap-2 sm:gap-4 lg:gap-6">
          {loadingLicores ? (
            ...
            <li
              ...
              className="text-secondary/60 text-center py-12 lg:py-16 text-base lg:text-lg font-light animate-fade-in-up opacity-0"
              ...
              {Object.entries(cantidades)
                ...
                .map(([id, cantidad]) => {
                  ...
                  if (cantidad.botellas > 0)
                    items.push(`${cantidad.botellas} bottle${cantidad.botellas > 1 ? 's' : ''}`);
                  if (cantidad.cajas > 0)
                    items.push(`${cantidad.cajas} case${cantidad.cajas > 1 ? 's' : ''}`);
