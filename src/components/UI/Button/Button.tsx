...
const buttonVariants = {
  primary:
    'px-6 py-3 bg-accent hover:bg-accentHover text-background rounded-xl font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2',

  secondary:
    'px-6 py-3 bg-border/50 hover:bg-border text-secondary hover:text-primary rounded-xl font-medium transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed',

  ghost:
    'bg-transparent hover:bg-transparent border-transparent hover:border-transparent text-secondary/60 hover:text-secondary transition-colors',

  filter:
    'px-3 sm:px-4 py-2 text-xs sm:text-sm font-medium transition-colors border-none rounded-none',

  danger:
    'bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 hover:border-red-500/50 text-red-400 hover:text-red-300 rounded-xl font-medium transition-all duration-200',

  destructive:
    'px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-medium transition-all duration-200',
  ...
  navTab:
    'px-4 sm:px-6 py-3 sm:py-4 text-sm sm:text-base font-medium transition-colors border-b-2 rounded-none bg-transparent hover:bg-transparent',

  quantityMinus:
    'w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-border/50 hover:bg-accent/20 text-primary transition-all duration-200 flex items-center justify-center font-light text-base sm:text-lg border border-border/20 hover:border-accent/30',

  quantityPlus:
    'w-8 h-8 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-accent hover:bg-accentHover text-background transition-all duration-200 flex items-center justify-center font-light text-base sm:text-lg shadow-lg hover:shadow-xl',

  clearSearch:
    'w-4 h-4 sm:w-5 sm:h-5 rounded-full bg-accent/20 hover:bg-accent/40 flex items-center justify-center text-accent transition-colors',

  dropdown:
    'w-full px-4 sm:px-6 py-3 sm:py-4 bg-cardBg border border-border rounded-xl sm:rounded-2xl text-primary focus:outline-none focus:border-accent/50 focus:ring-2 focus:ring-accent/20 transition-all text-left flex items-center justify-between hover:border-accent/30 text-sm sm:text-base',

  dropdownItem:
    'w-full px-4 sm:px-6 py-3 text-left text-sm sm:text-base transition-all duration-200 hover:bg-accent/10 hover:text-accent border-b border-border/20 rounded-none last:border-b-0',
};
...
const filterInactiveStyles =
  'bg-background/50 text-secondary/80 hover:text-secondary hover:bg-background/70 border-none';
...
export default function Button({
  ...
  const getVariantStyles = () => {
    switch (variant) {
      case 'filter':
        ...
        return isActive
          ? `${filterBase} ${filterActiveStyles}`
          : `${filterBase} ${filterInactiveStyles}`;
      case 'navTab':
  ...
  const baseStyles = shouldApplyBaseStyles
